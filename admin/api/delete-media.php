...
foreach ($input['paths'] as $relPath) {
    ...
    if (is_file($fullPath)) {
        if (unlink($fullPath)) {
            ...
            // If this is a gallery asset, update imageCount in users.js
            if (strpos($fullPath, 'gallery' . DIRECTORY_SEPARATOR . 'assets') !== false) {
                // Re-count images in the folder
            ...
            // If this is a portfolio asset, update portfolio.json
            if (strpos($fullPath, 'assets' . DIRECTORY_SEPARATOR . 'portfolio') !== false) {
                ...
                if (file_exists($portfolioFile)) {
                    ...
                    $data['images'] = array_values(array_filter($data['images'] ?? [], function($img) use ($filename) {
